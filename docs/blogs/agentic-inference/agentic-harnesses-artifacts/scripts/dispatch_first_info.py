...
def measure_one(url, model):
    ...
    body = {
        ...
        'stream': True,
        'messages': [
            {'role': 'system', 'content': 'You are a travel expert. Think about what makes each city special, then call echo.'},
    ...
    for line in r.iter_lines(decode_unicode=True):
        ...
        if line.startswith('event: '):
            ...
            if event_type == 'tool_call_dispatch' and result['dispatch_event_ms'] is None:
...
def main():
    ...
    for i in range(args.runs):
        ...
        dispatch = f"dispatch={r['dispatch_event_ms']:.1f}" if r['dispatch_event_ms'] else "no dispatch"
        tool = f"tool_delta={r['tool_delta_ms']:.1f}" if r['tool_delta_ms'] else "no tool delta"
        info = f"first_info={r['first_tool_info_ms']:.1f}" if r['first_tool_info_ms'] else "no info"
    ...
    if infos:
        print(f"\nFirst tool info: mean={statistics.mean(infos):.1f}ms  stdev={statistics.stdev(infos):.1f}ms", file=sys.stderr)
    if dones:
        print(f"Finish reason:   mean={statistics.mean(dones):.1f}ms  stdev={statistics.stdev(dones):.1f}ms", file=sys.stderr)
    if infos and dones:
        print(f"Info before done: {statistics.mean(dones) - statistics.mean(infos):.1f}ms avg", file=sys.stderr)
    ...
    if args.jsonl:
        with open(args.jsonl, 'w') as f:
            for r in results:
                f.write(json.dumps({k: round(v, 1) if isinstance(v, float) else v for k, v in r.items()}) + '\n')
