...
"""
import requests, json, time, copy, statistics, sys, uuid
...
def main():
    ...
    messages[0] = {'role': 'system', 'content': 'You are a travel expert. Rank cities by population when asked.'}
    ...
    print(f"Conversation: {len(messages)} messages, ~{total_chars} chars, ~{est_tokens} estimated tokens", file=sys.stderr)
    ...
    ratio = statistics.mean(mutated_times) / statistics.mean(exact_times) if statistics.mean(exact_times) > 0 else 0
