...
def run_conversation(url, model, cities):
    ...
    messages = [
        {
            "role": "system",
            "content": "You are a travel expert. For each city, think about what makes it special, then call the echo tool. Process one city at a time.",
        },
        {
            "role": "user",
            "content": f'Describe each city by calling echo: {", ".join(cities)}',
        },
    ]
    ...
    for turn in range(len(cities) + 5):  # extra turns for safety
        ...
        headers = {"Content-Type": "application/json"}
        resp = requests.post(
            f"{url}/v1/chat/completions", json=body, headers=headers, timeout=60
        )
        data = resp.json()
        ...
        print(
            f"  Turn {turn+1} ({city}): first_info={fi_str}ms  done={fr_str}ms  delta={info_before_done:.1f}ms  {'dispatch' if has_dispatch else 'no dispatch'}",
            file=sys.stderr,
        )
...
def main():
    ...
    print("Running 30-city conversation...", file=sys.stderr)
    ...
    if deltas:
        print(
            f"Info before done: mean={sum(deltas)/len(deltas):.1f}ms per turn",
            file=sys.stderr,
        )
        print(
            f"Cumulative earlier feedback: {sum(deltas):.0f}ms over {len(deltas)} turns",
            file=sys.stderr,
        )
